...
import (
	...
	"github.com/viacoin/viautil/bloom"
)
...
func TestFilterInsertKey(t *testing.T) {
	...
	wif, err := viautil.DecodeWIF(secret)
	if err != nil {
	...
	f.Add(viautil.Hash160(wif.SerializePubKey()))
...
func TestFilterBloomMatch(t *testing.T) {
	...
	tx, err := viautil.NewTxFromBytes(strBytes)
	if err != nil {
	...
	spendingTx, err := viautil.NewTxFromBytes(spendingTxBytes)
	if err != nil {
		t.Errorf("TestFilterBloomMatch NewTxFromBytes failure: %v", err)
		return
	}
...
func TestFilterInsertP2PubKeyOnly(t *testing.T) {
	...
	block, err := viautil.NewBlockFromBytes(blockBytes)
	if err != nil {
