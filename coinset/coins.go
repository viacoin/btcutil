...
func (cs *CoinSet) TotalValue() (value viautil.Amount) {
...
func satisfiesTargetValue(targetValue, minChange, totalValue viautil.Amount) bool {
...
type CoinSelector interface {
	CoinSelect(targetValue viautil.Amount, coins []Coin) (Coins, error)
}
...
type MinIndexCoinSelector struct {
	MaxInputs       int
	MinChangeAmount viautil.Amount
}

// CoinSelect will attempt to select coins using the algorithm described
// in the MinIndexCoinSelector struct.
func (s MinIndexCoinSelector) CoinSelect(targetValue viautil.Amount, coins []Coin) (Coins, error) {
	cs := NewCoinSet(nil)
...
func (s MinNumberCoinSelector) CoinSelect(targetValue viautil.Amount, coins []Coin) (Coins, error) {
...
func (s MaxValueAgeCoinSelector) CoinSelect(targetValue viautil.Amount, coins []Coin) (Coins, error) {
	sortedCoins := make([]Coin, 0, len(coins))
	sortedCoins = append(sortedCoins, coins...)
...
func (s MinPriorityCoinSelector) CoinSelect(targetValue viautil.Amount, coins []Coin) (Coins, error) {
...
// viautil.Tx, a specific outpoint index, and the number of confirmations
...
func (c *SimpleCoin) Value() viautil.Amount {
	return viautil.Amount(c.txOut().Value)
}
